...
export function getInputs(): ActionInputs {
  return {
    ...
    tools:
      core.getInput('tools') ||
      'patterns,context,consistency,ai-signal,grounding,testability,doc-drift,deps,change-amp',
...
export function setOutputs(results: AnalysisResult, passed: boolean): void {
  const score = results.scoring?.overall || 0;
  const totalIssues = results.summary.totalIssues || 0;
  const criticalCount = results.summary.criticalIssues || 0;
  const majorCount = results.summary.majorIssues || 0;
...
export async function run(): Promise<void> {
  try {
    ...
    const baseUrl =
      process.env.AIREADY_SERVER || 'https://platform.getaiready.dev';
    ...
    if (failOnRegression && apiKey) {
      try {
        ...
        if (response.ok) {
          ...
          core.warning(
            `⚠️ Failed to fetch baseline: ${response.statusText}. Using manual threshold.`
          );
        }
      } catch (err: any) {
        core.warning(
          `⚠️ Baseline error: ${err.message}. Using manual threshold.`
        );
      }
    }
    ...
    if (results.scoring?.costEstimate) {
      summary.addRaw(
        `**Estimated AI Waste:** $${results.scoring.costEstimate.total}/month (model: ${results.scoring.costEstimate.model})\n`
      );
    }
    ...
    if (uploadToSaas && apiKey) {
      const isProd =
        ...
        process.env.AIREADY_SERVER.includes('getaiready.dev');
      const baseUrl =
        process.env.AIREADY_SERVER || 'https://platform.getaiready.dev';
      const uploadUrl = `${baseUrl}/api/analysis/upload`;
